...
class SQLiteHook(BaseHook):
    '''
    Hook to log training metrics to SQLite.
    ...
    - Other shapes are stored as base64-encoded binary data.

    Args:
        project: The project name.
        entity: Not used.
        run_name: The name of the run. If not provided, a random name will be generated.
        config: A dictionary of hyperparameters to be logged.
        flush_interval: The interval (in steps) at which to flush the logged data to database.
    ...
    def __init__(
        ...
        from .. import logdb
        self.logdb = logdb
        self.local_db_storage = Path(local_db_storage)
    ...
    def _flush(self):
        if hasattr(self, 'sql_run'):
            pending = sorted(self.pending_data)
            for step in pending:
                data = self.pending_data[step]
                self.sql_run.log(
                    self._sync_tensor(data), step=step,
                    commit=step == pending[-1]
                ) # Flush at the last step to reduce overhead
    ...
    def _add_metrics(self):
        ...
        *_, metrics = self.parent.global_context.metrics[-1]
        # Align with the step at which losses are computed
        step = self.parent.global_context.step - 1
